...
Functions
---------
- ``linParts(numAtoms, numThreads)``
    Partition an integer range into contiguous pieces.
- ``nestedParts(numAtoms, numThreads, upperTriang=False)``
    Nested partitioning used for matrix-like tasks.
- ``mpPandasObj(func, pdObj, numThreads=24, mpBatches=1, linMols=True, **kargs)``
    Parallelize a pandas operation across cores.
- ``processJobs_`` and ``processJobs``
    Low level job processing utilities with progress reporting.
- ``expandCall(kargs)``
    Helper to unpack argument dictionaries.
- ``processJobsRedux`` and ``mpJobList``
    Additional helpers for iterative multiprocessing with reduction.
"""
...
def mpJobList(func, argList, numThreads, mpBatches = 1, linMols = True,
    ...
    return out
__all__ = ['linParts','nestedParts','mpPandasObj','processJobs_','reportProgress','processJobs','expandCall','processJobsRedux','mpJobList']
